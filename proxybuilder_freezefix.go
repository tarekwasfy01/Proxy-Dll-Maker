...
func guiMain() {
	...
	className := wide("ProxyBuilderFreezeFixClass")
	wc := wndClassEx{
	...
	hwnd, _, _ := procCreateWindowExW.Call(
		...
		uintptr(unsafe.Pointer(wide("ProxyBuilder - Freeze Fix"))),
...
func startWorker() {
	...
	if err != nil {
		setText(gui.hStatus, "Cannot resolve application path: "+err.Error())
		return
	}
	args := []string{
		exePath, "--worker",
	...
	proc, err := os.StartProcess(exePath, args, &os.ProcAttr{
		Env: os.Environ(),
		Sys: &syscall.SysProcAttr{HideWindow: true, CreationFlags: CREATE_NO_WINDOW},
	})
	if err != nil {
		setText(gui.hStatus, "Worker could not start: "+err.Error())
		return
	}
	gui.workerPID = proc.Pid
	_ = proc.Release() // no Wait() and no worker process handle leak in GUI.
	...
	setText(gui.hSummary, "Build work is fully isolated in a second process. Full details: "+gui.logPath)
}
